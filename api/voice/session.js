/* ═══════════════════════════════════════════════════════════════════════════
   ...
   Rate limit: VOICE_MINT_PER_HOUR per IP (default 6) — a voice session costs
   real money, so nobody runs one up from a URL. Plus the session cap the
   browser enforces (caps.sessionSeconds) and the silence mute.
   ═══════════════════════════════════════════════════════════════════════════ */
...
export function voiceConfig(env) {
  ...
  return {
    ...
    caps: {
      ...
      mintsPerHour: num(e.VOICE_MINT_PER_HOUR, 6),
...
export async function mintSession(body, env, fetchImpl) {
  ...
  const session = buildSession(DATA, e, { resume, page });
  ...
  try {
    ...
    return { status: 200, json: { ok: true, value: j.value, expiresAt: j.expires_at || null, model: cfg.model, voice: cfg.voice, caps: cfg.caps } };
  } catch (err) {
    clearTimeout(t);
...
export default async function handler(req, res) {
  ...
  const cfg = voiceConfig();
  if (limited(req, res, cfg.caps.mintsPerHour, 3600000)) return;
  let body = req.body;
