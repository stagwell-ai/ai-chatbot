/* ═══════════════════════════════════════════════════════════════════════════
   ...
   The chain is configuration, not code:
     ...
     KIMI_MODEL_TIMEOUT_MS default 4000   per attempt
     KIMI_TOTAL_DEADLINE_MS default 8000  for the whole chain
   ...
   satisfy the schema. At most ONE bounded retry per provider, and only for a
   retryable error with time left on the deadline; then the next model; then
...
export function chainConfig() {
  ...
  return {
    enabled,
    slots,
    timeoutMs: num('KIMI_MODEL_TIMEOUT_MS', 4000),
    deadlineMs: num('KIMI_TOTAL_DEADLINE_MS', 8000)
  };
}

export function buildChain(cfg, fetchImpl) {
  const c = cfg || chainConfig();
  return c.slots.map((s, i) => ({ slot: s, name: ['primary', 'secondary', 'tertiary'][i], provider: s ? providerFor(s, fetchImpl) : null }))
    .filter(x => x.provider);
}
...
export function describeChain(cfg) {
  ...
  return {
    ...
    chain: buildChain(c).map(x => ({ name: x.name, id: x.provider.id, vendor: x.provider.vendor, model: x.provider.model, configured: x.provider.configured }))
  };
}
...
export async function structured(req, opts) {
  ...
  for (let i = 0; i < chain.length; i++) {
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
      ...
      const r = await provider.complete({ system: req.system, user: req.user, maxTokens: req.maxTokens, json: req.json !== false, timeoutMs: Math.min(cfg.timeoutMs, left) });
      ...
      /* one bounded retry for a transient error, only with time in hand */
    }
    fallbacks++;
  }
