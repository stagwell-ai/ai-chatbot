...
/* every product carries its own ambient film (rendered Aug 26, from
   assets/seedance-carousel-prompts.txt) at /next-v1/assets/img/cloud/<id>.mp4 with
...
const CAMPAIGN_PAGES = {
  ...
  newvoices: '/next-v1/p/newvoices',
  agent_cloud: '/next-v1/p/agent-cloud'
};
function ourPage(p) {
  const id = p.sol || p.id;
...
const bigCard = p => `<a class="fcard fcard--dark fcard--video" href="${esc(ourPage(p))}">
    <span class="fcard__art"><video class="fcard__video" src="/next-v1/assets/img/cloud/${p.id}.mp4"
      poster="/next-v1/assets/img/cloud/${p.id}.jpg" ${AUTOPLAY} muted loop playsinline></video></span>
...
/* the small card leads with the company itself: icon, name, description.
   Redesign (Sep 3): each one also carries a small fragment of its own
   interface, centred on a soft stage — the pattern from the white build.
   No copy changes; the fragment is drawn, never captioned. */
const FRAG = {
  /* The animated fragments from the Catalogue build, reused verbatim.
     One per product, each looping a micro-interaction of its own UI. */
  bera:            () => bars([30,44,39,56,62,58,76,90]),
  questbrand:      () => bars([41,58,49,66,72,63,81,92]),
  harrisquest:     () => ask('Would buyers pay more for repairability?'),
  questdiy:        () => ask('How do buyers choose in this category?'),
  newindex:        () => rank(),
  unlock:          () => rank(),
  geopulse:        () => spark(),
  newintel:        () => feed(),
  ...
  agentcloud:      () => chips(),
  numetrix:        () => saydo(),
  'people-platform': () => saydo(),
  imai:            () => wave(),
  newvoices:       () => wave(),
  doreel:          () => film(),
};

/* the frames, exactly as the Catalogue draws them */
const ask = q => `<span class="cmini mini-ask">
    <svg viewBox="0 0 12 12" width="9" height="9"><circle cx="5.2" cy="5.2" r="3.6" fill="none"
      stroke="currentColor" stroke-width="1.4"/><path d="M8 8l2.6 2.6" stroke="currentColor"
      stroke-width="1.4" stroke-linecap="round"/></svg>
    <b>${esc(q)}</b><i class="mini-caret"></i></span>`;
const rank = () => `<span class="cmini mini-row">
    <b>#1</b>
    <span class="rankline"><i class="pulse"></i>in AI answers<em class="up">↑ 2</em></span></span>`;
const spark = () => `<span class="cmini mini-col">
    <svg viewBox="0 0 96 26" aria-hidden="true"><path class="mini-line"
      d="M2,22 L18,19 L34,21 L50,13 L66,15 L82,6 L94,3" fill="none" stroke="currentColor"
      stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"/></svg>
    <b>Answer share, 12 weeks</b></span>`;
const feed = () => `<span class="cmini mini-feed">
...
const bars = h => `<span class="cmini mini-bars">${
  h.map(v=>`<i style="--h:${v}%"></i>`).join('')}</span>`;
const wave = () => `<span class="cmini mini-wave">${Array.from({length:22},(_,i)=>
  `<i style="--h:${16+Math.round(Math.abs(Math.sin(i*0.72))*74)}%"></i>`).join('')}</span>`;
const film = () => `<span class="cmini mini-film">${
  [1,.74,.5,.3].map(o=>`<i style="--o:${o}"></i>`).join('')}</span>`;
...
const smCard = p => `<a class="scard" href="${esc(ourPage(p))}">
    ...
    <span class="scard__stage">${(FRAG[p.id] || spark)()}</span>
