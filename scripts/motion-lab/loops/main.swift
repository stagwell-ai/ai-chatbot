...
// 30 · radial: the reference. A huge ring, its centre off the left edge, the band bulging to the right;
// a tiny id per row; bars radiate outward to the right; the lower rows carry colour, and strands sweep
// in from the left, inside the ring, into those rows' chips on the band's inner edge
func radial(_ ctx: CGContext, _ p: Double) {
  ...
  func ang(_ i: Int) -> Double { -span / 2 + span * (Double(i) + 0.5) / Double(n) }
  func hot(_ i: Int) -> Bool { i >= 68 && i < 104 }
  func col(_ i: Int) -> CGColor { pal[Int(hash(i, 721) * Double(pal.count))] }
  // strands from the left into the band's outer edge
  for i in 0..<n {
    let a = ang(i), ex = cx + cos(a) * R, ey = cy + sin(a) * R
    let cnt = hot(i) ? 4 : 1
    for k in 0..<cnt {
  ...
  ctx.setStrokeColor(white(0.35)); ctx.setLineWidth(1.5 * K)
  ctx.addEllipse(in: CGRect(x: cx - R, y: cy - R, width: 2 * R, height: 2 * R)); ctx.strokePath()
  // rows: chip, id, bar (in a frame whose x runs outward from the band's inner edge)
