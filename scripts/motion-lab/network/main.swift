...
do {
  ...
  for (rank, (_, i)) in keyed.enumerated() {
    ...
    appear[i] = 0.1 + 2.2 * pow(u, 0.9)       // seconds: built by ~2.5 s
  }
}
let BUILT = 2.8
...
render(path: CommandLine.arguments[1], w: W, h: H, fps: FPS, frames: FR) { ctx, f in
  ...
  let z = 1.15 - 0.15 * ease(sec / 3.2)
  ctx.translateBy(x: Double(W) / 2, y: Double(H) / 2)
  ctx.scaleBy(x: z, y: z)
  ctx.translateBy(x: -Double(W) / 2, y: -Double(H) / 2)
  let alive = max(0, min(1, (sec - BUILT + 0.5) / 0.5))

  ctx.setLineWidth(0.9)
  for e in edges {
    let start = max(appear[e.a], appear[e.b]) + 0.15
    let g = ease((sec - start) / 0.3)
    if g <= 0.01 { continue }
